...
namespace NbaPredictor.Test
{
    public class TurnoverStatsProviderTests
    {
        ...
        public async Task GetStatsReturnsStatsFromWebProvider()
        {
            ...
            httpClient.BaseAddress = new Uri("https://www.teamrankings.com/nba/stat/turnovers-per-game");
            var statsProvider = new TurnoverStatsProvider(httpClient);
            var result = await statsProvider.GetStats();
            //result.First().Should().ContainKey("Team").WhichValue.Should().Be("San Antonio");
